...
class SequencePanel(QGroupBox):
    ...
    def _load_file(self) -> None:
            ...
            # Load without channel validation first — we'll map by position
            try:
            ...
            self.log_label.setText(
                f'Loaded {self._step_count} steps.'
            )
            self._set_controls_enabled(True)

            # Emit channel names so PumpPanel fills Contents
            self.channels_loaded.emit(channel_names)
            # Show first step setpoints but don't start yet
            self.channels_loaded.emit(channel_names)
    ...
    def start(self) -> None:
        ...
        if self._step_count > 0 and not (self._worker and self._worker.isRunning()):
